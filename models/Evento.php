...
class Evento extends ActiveRecord
{
    ...
    public function validar()
    {
        ...
        if (
            !$this->categoria_id ||
            !filter_var($this->categoria_id, FILTER_VALIDATE_INT)
        ) {
        ...
        if (
            !$this->hora_id ||
            !filter_var($this->hora_id, FILTER_VALIDATE_INT)
        ) {
            self::$alertas['error'][] = 'Elige la hora del evento';
        }
        if (
            !$this->disponibles ||
            !filter_var($this->disponibles, FILTER_VALIDATE_INT)
        ) {
            self::$alertas['error'][] =
                'Añade una cantidad de Lugares Disponibles';
        }
        if (
            !$this->restaurante_id ||
            !filter_var($this->restaurante_id, FILTER_VALIDATE_INT)
        ) {
            self::$alertas['error'][] = 'Selecciona el restaurante del evento';
        }
